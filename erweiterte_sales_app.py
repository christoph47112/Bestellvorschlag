...
def average_sales_app():
    ...
    if uploaded_file:
        ...
        # Überprüfen, ob relevante Spalten existieren und ggf. umbenennen
        if 'Unnamed: 0' in df.columns and 'Artikel' in df['Unnamed: 0'].values:
            df.columns = df.iloc[0]  # Erste Zeile als Header setzen
            df = df[1:]  # Erste Zeile entfernen
            df.rename(columns={'Artikel': 'Artikelnummer', 'Name': 'Artikelname', 'Menge': 'Menge'}, inplace=True)

        # Erweiterte Datenvalidierung
        required_columns = {"Artikelnummer", "Woche", "Menge", "Artikelname"}
        if not required_columns.issubset(df.columns):
            st.error("Fehler: Die Datei muss die Spalten 'Artikelnummer', 'Woche', 'Menge' und 'Artikelname' enthalten.")
            ...
            if artikel_filter:
                df = df[df['Artikelnummer'].astype(str).str.contains(artikel_filter, case=False, na=False)]

            if artikel_name_filter:
                df = df[df['Artikelname'].str.contains(artikel_name_filter, case=False, na=False)]

            # Durchschnittliche Abverkaufsmengen berechnen und Originalreihenfolge beibehalten
            result = df.groupby(['Artikelnummer', 'Artikelname'], sort=False).agg({'Menge': 'mean'}).reset_index()
            ...
            if st.checkbox("Vergleiche mit einer anderen Datei anzeigen"):
                ...
                if uploaded_file_compare:
                    ...
                    compare_result = compare_data.groupby(['Artikelnummer', 'Artikelname']).agg({'Menge': 'mean'}).reset_index()
                    ...
                    merged_results = result.merge(compare_result, on='Artikelnummer', suffixes=('_Original', '_Vergleich'))
