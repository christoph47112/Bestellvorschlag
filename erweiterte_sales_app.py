...
def bestellvorschlag_app():
    ...
    if abverkauf_file and bestand_file:
        abverkauf_data = pd.ExcelFile(abverkauf_file)
        sheet_name = st.sidebar.selectbox("Wählen Sie das Blatt für Abverkäufe aus", abverkauf_data.sheet_names)
        abverkauf_df = abverkauf_data.parse(sheet_name)
        bestand_data = pd.ExcelFile(bestand_file)
        sheet_name = st.sidebar.selectbox("Wählen Sie das Blatt für Bestände aus", bestand_data.sheet_names)
        bestand_df = bestand_data.parse(sheet_name)
